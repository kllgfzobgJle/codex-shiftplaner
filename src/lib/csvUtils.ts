...
export function csvToEmployees(
  csvContent: string,
...
export function csvToShiftTypes(
  csvContent: string,
): Omit<ShiftType, "id" | "createdAt" | "updatedAt">[] {
  const lines = csvContent.split("\n").filter((line) => line.trim());
  if (lines.length < 2)
    throw new Error("CSV file must have at least a header and one data row");
...
export function learningYearQualificationsToCSV(
  qualifications: LearningYearQualification[],
): string {
  const headers = [
    "id",
    "jahr",
...
export function csvToLearningYearQualifications(
  csvContent: string,
): Partial<LearningYearQualification>[] {
  const lines = csvContent.split("\n").filter((line) => line.trim());
  if (lines.length < 2)
    throw new Error("CSV file must have at least a header and one data row");
...
export function csvToShiftRules(
  csvContent: string,
): Omit<ShiftRule, "id" | "createdAt" | "updatedAt">[] {
  const lines = csvContent.split("\n").filter((line) => line.trim());
  if (lines.length < 2)
    throw new Error("CSV file must have at least a header and one data row");
